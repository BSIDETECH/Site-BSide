...
export default function RestaurantTemplate({ data }: { data: RestaurantData }) {
  return (
    <div className={`min-h-screen font-sans selection:bg-red-500 selection:text-white ${data.colors.background} ${data.colors.text}`}>
      {/* Hero Section */}
      <header className="relative pt-16 pb-20 lg:pt-24 lg:pb-32 overflow-hidden px-4">
        ...
        <div className="max-w-4xl mx-auto flex flex-col items-center text-center relative z-10">
          ...
          <div className="mb-8 flex flex-col items-center justify-center p-6 bg-white rounded-full shadow-[0_0_40px_rgba(220,38,38,0.3)] w-48 h-48 sm:w-56 sm:h-56">
            <div className={`w-16 h-16 rounded-full border-4 border-white mb-2 flex items-center justify-center ${data.colors.accent} text-white shadow-inner`}>
            ...
            <p className="text-xs sm:text-sm font-bold text-slate-600 uppercase tracking-widest mt-1">{data.subtitle}</p>
          </div>

          <h2 className="text-4xl md:text-5xl font-extrabold mb-6 leading-tight drop-shadow-lg">
            {data.heroTitle}
          </h2>
          <p className="text-lg md:text-xl text-slate-300 max-w-2xl mx-auto mb-10 opacity-90">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="px-8 py-4 rounded-xl font-bold flex items-center justify-center gap-2 bg-slate-800 text-white hover:bg-slate-700 transition-colors border border-slate-700 shadow-lg"
            >
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {data.products.map((product, index) => (
              <div key={index} className="bg-slate-950 rounded-2xl overflow-hidden border border-slate-800 hover:border-slate-600 transition-colors group">
                <div className="h-48 w-full bg-slate-900 relative overflow-hidden">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-slate-800 group-hover:scale-105 transition-transform duration-500">
                    <span className="text-sm uppercase font-semibold tracking-wider text-slate-500">Foto {product.name}</span>
                  </div>
                </div>
                <div className="p-6 relative z-20 -mt-8 bg-slate-950 rounded-t-2xl">
                  <div className="flex justify-between items-start mb-3">
                    <h4 className="text-xl font-bold text-white pr-4">{product.name}</h4>
                    <span className={`font-black whitespace-nowrap px-3 py-1 bg-slate-900 rounded-lg border border-slate-800 ${data.colors.priceText}`}>{product.price}</span>
                  </div>
        ...
        <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {data.reviews.map((review, index) => (
              <div key={index} className="bg-slate-900/90 backdrop-blur-sm p-8 rounded-2xl border border-slate-800 text-left shadow-xl">
                <p className="text-slate-300 italic mb-6 text-lg">"{review.text}"</p>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
            <div>
              ...
              <p className="text-slate-400 font-medium">{data.address.street}</p>
              <p className="text-slate-400">{data.address.city}</p>
            </div>
            <div>
              <h4 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
              ...
              <div className="space-y-2">
                {data.hours.map((hour, index) => (
                  <p key={index} className="text-slate-400 flex justify-between max-w-[250px] border-b border-slate-800 pb-2">
            ...
            <div>
              ...
              <p className="text-slate-400 mb-4 font-medium">{data.contact.phone}</p>
              <a
                href={data.links.whatsapp}
                target="_blank"
                rel="noopener noreferrer"
                className={`inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-slate-900 border border-slate-700 text-white font-medium hover:bg-slate-800 transition-colors w-full sm:w-auto justify-center`}
              >
                Chamar no WhatsApp
              </a>
            </div>
          ...
          <div className="pt-8 border-t border-slate-900 text-center text-sm text-slate-500 flex flex-col sm:flex-row justify-between items-center">
            <p>&copy; {new Date().getFullYear()} {data.name}. Todos os direitos reservados.</p>
            <p className="mt-4 sm:mt-0">
              Orgulhosamente desenvolvido por <a href="https://somosbside.com.br" className="text-slate-400 hover:text-white font-bold transition-colors" target="_blank" rel="noopener noreferrer">B-Side Tech</a>
            </p>
          </div>
        </div>
      </footer>
